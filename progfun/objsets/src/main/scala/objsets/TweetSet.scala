...
abstract class TweetSet {
  ...
  def ascendingByRetweet: Trending = doAscendingByRetweet(new EmptyTrending)
  def doAscendingByRetweet(acc: Trending): Trending
...
class Empty extends TweetSet {
  ...
  def doAscendingByRetweet(acc: Trending): Trending = acc
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = {
    ...
    left.filter0(p, right.filter0(p, newAccu))
  }
  ...
  def doAscendingByRetweet(acc: Trending): Trending = {
    val min = findMin
    remove(min).doAscendingByRetweet(acc + min)
  }
